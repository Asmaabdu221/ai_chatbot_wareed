...
const WELCOME_TEXT =
  'مرحباً بك في تجربة البوت الذكي. اكتب سؤالك وسأقوم بالرد مباشرة.';
const TYPING_TEXT = 'جاري الكتابة...';
const ERROR_TEXT = 'تعذر الاتصال حالياً، حاول مرة أخرى بعد قليل.';
...
export default function DashboardChatWidget() {
  ...
  return (
    <div className="dashboard-chat-widget" dir="rtl" lang="ar">
      <button
        type="button"
        className="dashboard-chat-widget__trigger"
        aria-expanded={isOpen}
        onClick={() => setIsOpen((prev) => !prev)}
        title="تجربة البوت الذكي"
      >
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M4 5h16a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-8l-4 3v-3H4a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z" />
        </svg>
      </button>

      {isOpen && (
        <aside className="dashboard-chat-widget__panel" aria-label="تجربة البوت الذكي">
          <header className="dashboard-chat-widget__header">
            <h3>تجربة البوت الذكي</h3>
            <button
              type="button"
              className="dashboard-chat-widget__close"
          ...
          <div className="dashboard-chat-widget__meta">
            <span>user: {sessionUserId ? sessionUserId.slice(0, 8) : '—'}</span>
            <span>conv: {sessionConversationId ? sessionConversationId.slice(0, 8) : '—'}</span>
          </div>

          <div ref={listRef} className="dashboard-chat-widget__messages">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`dashboard-chat-widget__bubble dashboard-chat-widget__bubble--${message.role}`}
              >
          ...
          <form className="dashboard-chat-widget__form" onSubmit={onSubmit}>
            <input
              type="text"
              value={input}
