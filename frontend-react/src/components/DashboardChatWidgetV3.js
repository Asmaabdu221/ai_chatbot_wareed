...
function renderText(text) {
  ...
  const brand = 'Wareed AI';

  return lines.map((line, index) => (
    <React.Fragment key={`line-${index}`}>
      {line.split(brand).map((part, partIndex, parts) => (
        <React.Fragment key={`part-${index}-${partIndex}`}>
          {part}
          {partIndex < parts.length - 1 ? (
            <span className="dashboard-chat-widget-v3__brand">{brand}</span>
          ) : null}
        </React.Fragment>
      ))}
...
export default function DashboardChatWidgetV3() {
  ...
  const [messages, setMessages] = useState([
    { id: 'welcome-typing', role: 'assistant', text: TYPING_TEXT, typing: true },
  ]);
  ...
  return (
    <div className="dashboard-chat-widget-v3" dir="rtl" lang="ar">
      <button
        type="button"
        className="dashboard-chat-widget-v3__trigger"
      ...
      {isOpen && (
        <aside className="dashboard-chat-widget-v3__panel" aria-label="تجربة البوت الذكي">
          <header className="dashboard-chat-widget-v3__header">
            <h3>Wareed AI</h3>
            <button
              type="button"
              className="dashboard-chat-widget-v3__close"
          ...
          <div ref={listRef} className="dashboard-chat-widget-v3__messages">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`dashboard-chat-widget-v3__bubble dashboard-chat-widget-v3__bubble--${message.role} dashboard-chat-widget-v3__bubble--enter`}
              >
                {message.typing ? (
                  <span className="dashboard-chat-widget-v3__typing" aria-label="جاري الكتابة">
                    <span />
                    <span />
                    <span />
                  </span>
          ...
          <form className="dashboard-chat-widget-v3__form" onSubmit={onSubmit}>
            <div className="dashboard-chat-widget-v3__input-wrap">
              <input
                type="text"
                value={input}
