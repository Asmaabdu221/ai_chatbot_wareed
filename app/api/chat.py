    ...
    *,
    ...
    try:
        ...
        prompt_template = (
            ...
            "الهدف من الملخص:\n"
            "أن يفهم الموظف بسرعة:\n"
            "- ماذا أراد العميل بالضبط\n"
            "- ما المواضيع التي سأل عنها\n"
            "- ما التفاصيل المهمة التي ذكرها\n"
            "- هل حصل خطأ أو نقص في الرد\n"
            "- هل طلب متابعة أو ترك رقمه\n\n"
            "قواعد مهمة جدًا:\n"
            "1. اكتب الملخص بالعربية فقط.\n"
            "2. لا تستخدم الإنجليزية إطلاقًا.\n"
            "3. لا تكتب كلامًا عامًا أو مبهمًا.\n"
            "4. لا تختصر أكثر من اللازم.\n"
            "5. لا تكرر الحوار حرفيًا.\n"
            "6. لا تضف أي معلومة غير موجودة في المحادثة.\n"
            "7. لا تخلط بين العملاء أو المحادثات.\n"
            "8. اعتمد فقط على رسائل هذا العميل في نفس conversation_id.\n"
            "9. اذكر التفاصيل المهمة حتى لو كانت متعددة.\n"
            "10. إذا لم يذكر العميل شيئًا محددًا، فاذكر أنه استفسار عام.\n\n"
            "ما الذي يجب أن يشمله الملخص؟\n"
            "- نوع أو أنواع الاستفسار:\n"
            "  - تحليل\n"
            "  - باقة\n"
            "  - سعر\n"
            "  - فروع\n"
            ...
            "  - استفسار عام\n\n"
            "- إذا ذكر العميل:\n"
            "  - اسم تحليل محدد، اذكره\n"
            "  - اسم باقة، اذكرها\n"
            "  - مدينة أو فرع، اذكرها\n"
            "  - أعراض، اذكر الأعراض نفسها\n"
            "  - سؤال عن السعر، اذكر أنه سأل عن السعر\n"
            "  - طلب تواصل أو خدمة عملاء، اذكر ذلك\n"
            "  - رقم هاتف، اذكر أنه ترك رقمه للمتابعة\n\n"
            "- إذا انتقل العميل بين أكثر من موضوع، اذكر هذا التسلسل بوضوح.\n"
            "- إذا اختار العميل فرعًا أو مدينة، اذكرها صراحة.\n"
            "- إذا حصل خطأ أو لم يتم الرد على نقطة معينة، اذكر ذلك بوضوح.\n"
            "- إذا انتهت المحادثة بترك الرقم، اختم بهذا بوضوح.\n\n"
            "أسلوب الكتابة المطلوب:\n"
            "- من سطر إلى 3 أسطر كحد أقصى\n"
            "- واضح، طبيعي، ومفيد للموظف\n"
            "- ليس رسميًا بشكل مبالغ فيه\n"
            "- ليس عامًا أو سطحيًا\n\n"
            "صيغة التلخيص المطلوبة:\n"
            "ابدأ مباشرة بملخص ما حدث مثل:\n"
            "- العميل استفسر عن...\n"
            "- العميل بدأ بالسؤال عن... ثم انتقل إلى...\n"
            "- العميل ذكر أعراض... وسأل عن...\n"
        ...
        res = openai_service.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "أنت كاتب ملخصات عربي احترافي. التزم فقط بمحتوى المحادثة المرسلة.",
                },
            ...
            if _runtime_result.get("matched"):
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "dialogue_manager | state_after_update | conversation_id=%.8s | active_domain=%s | active_entity_name=%s | active_result_test_name=%s | active_result_value=%s",
                            str(conversation_id),
                            str(_dm_saved_state.get("active_domain") or ""),
                            str(_dm_saved_state.get("active_entity_name") or ""),
