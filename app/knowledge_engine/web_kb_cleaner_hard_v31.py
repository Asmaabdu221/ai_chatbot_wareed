...
BANNED_PATTERNS = [
    ...
    r"\bخلال\s*48\b",
    r"\b48\s+ساعة\b",
    # v3.1: remove marketing slogans / non-CTA promo language
    r"\bراقب\s+قلبك\b",
    r"\bاحم\s+نفسك\b",
    r"\b(?:و)?اكتشف\b",
    r"\b(?:بنسبة\s+)?دقة\s+تصل\s+(?:إلى|الى)\s*[0-9٠-٩]+\s*[%٪]\b",
    r"\bدقة\s+عالية\b",
    r"\bبدقة\s+عالية\b",
    r"\bموثوقية\b",
    r"\bسريعة\b",
...
def _strip_banned_global(text: str) -> str:
    ...
    for literal in [
        ...
        "نستخدم أجهزة",
        "نوفر لك",
        "بنسبة دقة تصل إلى 99%",
        "بنسبة دقة تصل الى 99%",
        "بدقة تصل إلى 99%",
        "بدقة تصل الى 99%",
    ]:
        out = out.replace(literal, " ")
    # Remove slogan-like fragments with ellipsis/exclamation overuse.
    out = re.sub(r"[.!؟!]{3,}", " ", out)
    out = re.sub(r"(?:^|\s)[^.!؟!\n]*!{2,}[^.!؟!\n]*(?:$|\s)", " ", out)
    return _normalize_punct(out)


def _normalize_title_like(text: str) -> str:
    out = _strip_banned_global(text)
    out = re.split(r"[|｜¦]+", out, maxsplit=1)[0].strip()
    out = re.sub(r"\s*[|｜¦]+\s*$", "", out).strip()
    out = re.sub(r"\b(?:في|مع|و)\s*$", "", out).strip()
    return _normalize_punct(out)
...
def _post_clean_repair(text: str) -> str:
    ...
    out = out.replace("الأطعمةمة", "الأطعمة").replace("متقدمةم", "متقدمة")
...
def _canonicalize_token(raw: str) -> str | None:
    key = re.sub(r"[^a-z0-9]", "", raw.lower())
    if key in DISEASE_TOKEN_BLOCKLIST:
        return None
    return LAB_TOKEN_CANONICAL.get(key)


def _extract_primary_codes(title: str, h1: str, content: str) -> list[str]:
    candidates: list[str] = []
    # v3.1 policy: title/h1 first.
    for src in [title, h1]:
        for m in re.finditer(r"\(([A-Za-z][A-Za-z0-9\-\s]{1,24})\)", src or ""):
            candidates.append(m.group(1).strip())
        candidates.extend(re.findall(r"\b[A-Z][A-Z0-9\-]{1,10}\b", src or ""))
    # body only for explicit patterns.
    explicit = [
        r"رمز[^.\n]{0,40}?هو\s+([A-Za-z][A-Za-z0-9\-\s]{1,16})",
        r"اختصار[^.\n]{0,40}?([A-Za-z][A-Za-z0-9\-\s]{1,16})",
    ]
    for pat in explicit:
        for m in re.finditer(pat, content or "", flags=re.IGNORECASE):
            candidates.append(m.group(1).strip())

    out: list[str] = []
    seen: set[str] = set()
    for c in candidates:
        canon = _canonicalize_token(c)
...
def _slug_fallback_test_name(url: str) -> str:
    slug = urlparse(url).path.strip("/").lower()
    if "vitamin-c" in slug:
        return "فيتامين C"
    if "insulin-resistance" in slug:
        return "مقاومة الإنسولين"
    if "homocysteine" in slug:
        return "تحليل الهوموسيستين"
    return ""


def _is_malformed_name(text: str) -> bool:
    t = _normalize_punct(text)
    # e.g. "c تحليل فيتامين" or starts with single latin token.
    return bool(re.match(r"^[A-Za-z]\b", t)) or bool(re.match(r"^[A-Za-z]\s+.*(?:تحليل|فحص)", t))
...
def clean_site_knowledge_jsonl(input_path: Path, clean_out: Path, chunks_out: Path) -> dict:
    ...
    for row in rows:
        ...
        title = _normalize_title_like(raw_title)
        h1 = _normalize_title_like(raw_h1)
        ...
        base_test_name = _normalize_punct(re.sub(r"^\s*(?:تحليل|فحص|اختبار)\s+", "", (h1 or title or "").strip()))
        test_name = _normalize_title_like(base_test_name)
        if _is_malformed_name(test_name):
            title_based = _normalize_title_like(re.sub(r"^\s*(?:تحليل|فحص|اختبار)\s+", "", title))
            test_name = title_based or _slug_fallback_test_name(url) or test_name
        test_name = _normalize_title_like(test_name)

        codes = _extract_primary_codes(title, h1, raw_content)
        ...
        docs.append(
            {
                ...
                "title": title,
                "h1": h1,
                "test_name_ar": _normalize_title_like(test_name)[:140],
                "test_code_tokens": codes[:2],
